"""Console script entrypoint for the Phase 4 NLP framing pipeline."""
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Parse command-line arguments for Phase 4 framing enrichment."""
    parser = argparse.ArgumentParser(
        description=(
            "Enrich silver.fact_mention_nlp_summary with primary frames and "
            "materialize silver.fact_mention_frame_score."
        ),
    )
    parser.add_argument(
        "--nlp-input-path",
...
def main(argv: list[str] | None = None) -> int:
    """Run the NLP framing pipeline and return a process exit code."""
    ...
    try:
        ...
        logger.exception("NLP framing pipeline exited with failure")
        return 1

    logger.info(
        "NLP framing pipeline finished status=%s run_id=%s artifacts=%d",
        result.status,
        result.run_id,
