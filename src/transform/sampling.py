...
def _sample_stratum(
    ...
    sampled_f = female_pool.sample(n=per_gender, random_state=random_seed)
    sampled_m = male_pool.sample(n=per_gender, random_state=random_seed)
...
def _attempt_geographic_resampling(
    ...
    for absent_reg in list(absent_regions):
        ...
        over_rep_region = region_counts[
            region_counts == region_counts.max()
        ].index[0]
        ...
        sample_df = pd.concat(
            [sample_df, swap_in.to_frame().T], ignore_index=True
        )
...
def _validate_sample(sample_df: pd.DataFrame) -> None:
    ...
    if total != CANDIDATE_SAMPLE_SIZE:
        raise SamplingError(
            f"Sample size {total} != target {CANDIDATE_SAMPLE_SIZE}"
        )
    ...
    if male_count != target_per_gender:
        raise SamplingError(
            f"Male count {male_count} != target {target_per_gender}"
        )
...
def _write_sample_manifest(
    ...
    Args:
        sample_df: The final sampled DataFrame.
    ...
    """
    by_gender = sample_df["gender"].value_counts().to_dict()
    by_city_size = sample_df["city_size_bucket"].value_counts().to_dict()
    by_nuance = sample_df["nuance_group"].value_counts().to_dict()
    region_codes = sorted(sample_df["reg_code"].dropna().unique().tolist())
        ...
        candidates_list.append(
            {
                ...
                "is_incumbent": bool(row.get("is_incumbent")) if row.get("is_incumbent") is not None else None,
...
def build_sample(
    ...
    logger.info(
        "Loaded dim_candidate_leader rows=%d", len(all_leaders_df)
    )
    ...
    logger.info("Gold Parquet written path=%s rows=%d", gold_parquet_path, len(final_sample_df))
    ...
    try:
        ...
        logger.info(
            ...
            conn.execute("SELECT count(*) FROM gold.sample_leaders").fetchone()[0],
        )
    finally:
        conn.close()
